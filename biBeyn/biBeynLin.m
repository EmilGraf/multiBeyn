function eigs = biBeynLin(P1,P2,opts)
% Bivariate Beyn's method based on Grothendieck residue, linear version

% Usage: eigs = biBeynLin(P1, P2, opts)
% - P1, P2 are the 3D coefficient tensors for a linear 2PEP
...
P1z1 = P1(:,:,2,1);  P1z2 = P1(:,:,1,2);  P1c = P1(:,:,1,1);
P2z1 = P2(:,:,2,1);  P2z2 = P2(:,:,1,2);  P2c = P2(:,:,1,1);

[A0,A1,A2] = beyn(P1z1,P1z2,P1c,P2z1,P2z2,P2c,u1,v1,u2,v2,opts);

eigs = matcheigs(A0,A1,A2,P1z1,P1z2,P1c,P2z1,P2z2,P2c,opts,1e-1);

end

% =========================================================================
function [A,B,C] = beyn(P1z1,P1z2,P1c,P2z1,P2z2,P2c,u1,v1,u2,v2,opts)
...
d1z1 = @(J) kronapp(P1z1', I2, J);
d1z2 = @(J) kronapp(P1z2', I2, J);
d2z1 = @(J) kronapp(I1, P2z1', J);
d2z2 = @(J) kronapp(I1, P2z2', J);

% Evaluate P1,P2 quickly at (z1,z2)
p1eval = @(z1,z2) P1z1*z1 + P1z2*z2 + P1c;
p2eval = @(z1,z2) P2z1*z1 + P2z2*z2 + P2c;
...
parfor i = 1:numel(pts)
    ...
    for j = 1:numel(pts)
        ...
        for k = 1:numel(gpts)
            ...
            J11 = syl(Q2aZ,Q2aT,Q2dZ,Q2dT,x1,pagetranspose(x2)).'*d2z2(J);

            % J12 = (I1 ⊗ P2z') * syl(Q1a,Q1d, d1z2(J))
            % J12 = kronapp(I1, P2z', syl(Q1aZ,Q1aT,Q1dZ,Q1dT,d1z2(J)));
            [x1,x2] = kronappk(I1,P2z',u2,v2);

            J12 = syl(Q1aZ,Q1aT,Q1dZ,Q1dT,x1,pagetranspose(x2)).'*d1z2(J);
            J = J11 - J12;
...
parfor i = 1:numel(pts)
    ...
    for j = 1:numel(pts)
        ...
        for k = 1:numel(gpts)
            ...
            J21 = syl(Q1aZ,Q1aT,Q1dZ,Q1dT,x1,pagetranspose(x2)).'*d1z1(J);

            % J22 = (P1z') ⊗ I2 * syl(Q2a,Q2d, d2z1(J))
            % J22 = kronapp(P1z', I2, syl(Q2aZ,Q2aT,Q2dZ,Q2dT,d2z1(J)));  
            [x1,x2] = kronappk(P1z',I2,u2,v2);

            J22 = syl(Q2aZ,Q2aT,Q2dZ,Q2dT,x1,pagetranspose(x2)).'*d2z1(J);
...
function eigs = matcheigs(A0,A1,A2,P1z1,P1z2,P1c,P2z1,P2z2,P2c,opts,tol)
    ...
    for i = 1:size(e1)
        e21 = eig(P1z1*e1(i)+P1c,-P1z2);
        e22 = eig(P2z1*e1(i)+P2c,-P2z2);
        i1 = abs(e21) < 1;
        i2 = abs(e22) < 1;
        e21 = e21(i1);
        e22 = e22(i2);
    ...
    for i = 1:size(eigs,1)
        eval(i) = min(svd(p1eval(eigs(i,1),eigs(i,2))))+min(svd(p2eval(eigs(i,1),eigs(i,2))));
    end
    ind = eval < tol;
    eigs = eigs(ind,:);
