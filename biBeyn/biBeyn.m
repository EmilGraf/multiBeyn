function eigs = biBeyn(P1,P2,dP11,dP12,dP21,dP22,opts)
% Bivariate Beyn's method based on Grothendieck residue, analytic version

% Usage: eigs = biBeyn(P1, P2, opts)
% - P1, P2 are function handles for the 2D analytic matrix functions
% - dPij are fnuction handles for the partial derivatives
% - opts (optional): struct with fields
% .tol (default 1e-4)
% .p (Gauss-Legendre nodes per radial; default 50)
% .block (random test block size; default k at the moment)
% .mom (size of Hankel problem to consruct with moments)

if nargin < 7, opts = struct; end
if ~isfield(opts,'tol'), opts.tol = 1e-4; end
if ~isfield(opts,'p'), opts.p = 50; end
if ~isfield(opts,'mom'), opts.mom = 3; end
if ~isfield(opts,'h'), opts.h = 1e-6; end

% dimensions
n1 = size(P1(0,0),1); n2 = size(P2(0,0),1); k = n1*n2;
...
[A0,A1,A2] = beyn(P1,P2,dP11,dP12,dP21,dP22,u1,v1,u2,v2,opts);

eigs = matcheigs(A0,A1,A2,P1,P2,opts,1e-1);
end

% =========================================================================
function [A,B,C] = beyn(P1,P2,dP11,dP12,dP21,dP22,u1,v1,u2,v2,opts)
...
I1 = eye(size(P1(0,0),1));
I2 = eye(size(P2(0,0),1));

% ---- accumulators
s = [size(u1,3) size(u1,3) m];
A = zeros(s(1:2));  B = zeros(s);  C = B;

% ---- Jacobian “partials” don’t depend on (z1,z2) for linear P1,P2
%      Write them as small closures that simply kron-apply constants.
d1z1 = @(z1,z2,J) kronapp(dP11(z1,z2)', I2, J);
d1z2 = @(z1,z2,J) kronapp(dP12(z1,z2)', I2, J);
d2z1 = @(z1,z2,J) kronapp(I1, dP21(z1,z2)', J);
d2z2 = @(z1,z2,J) kronapp(I1, dP22(z1,z2)', J);
...
function eigs = matcheigs(A0,A1,A2,P1,P2,opts,tol)
    ...
    for i = 1:size(e1)
        e21 = unibeyn(P1,e1(i),opts);
        e22 = unibeyn(P2,e1(i),opts);
        i1 = abs(e21) < 1;
        i2 = abs(e22) < 1;
        e21 = e21(i1);
        e22 = e22(i2);
        ...
        if numel(e21) > 0 && numel(e22) > 0
            for j = 1:size(e21)
                i1(j) = min(abs(e21(j)-e22),[],"all");
            end
            i1 = (i1 < tol);
            e21 = e21(i1);
            eigs = [eigs; repmat(e1(i),sum(i1),1) e21];
        end
    end
    
    % residual check
    eval = zeros(size(eigs,1),1);
    for i = 1:size(eigs,1)
        eval(i) = min(svd(P1(eigs(i,1),eigs(i,2))))+min(svd(P2(eigs(i,1),eigs(i,2))));
    end
    ind = eval < tol;
    eigs = eigs(ind,:);
...
%%% =========================================================================
% Univariate Beyn, given the first coordinate of an eigenvalue, finds the
% second using univariate Beyn's method

function eigs = unibeyn(P,x,opts)
n = opts.p;
tol = opts.tol;
...
A0 = zeros(d,k);
A1 = zeros(d,k);
V = randn(d,k) + 1i*randn(d,k);
for j = 1:n %quad1
    A0 = A0 + (f(quad(j)))\V.*quad(j);
    A1 = A1 + (f(quad(j)))\V.*dquad(j);
end
A0 = A0/n;
A1 = A1/n;
...
%Project
[R,S,VV] = svd(A0,"econ");
ind = find(abs(diag(S)/S(1,1))>tol,1,'last');
A = R(:,1:ind)'*A1*VV(:,1:ind);

eigs = eig(A,S(1:ind,1:ind));
end
